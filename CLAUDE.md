...
The one exception is `worker.js` (see deployment note above) — a thin router whose `fetch` handler special-cases `/api/link-preview` (a single stateless server-side `fetch` of a pasted link, returning its Open Graph title/description/image, purely because the browser itself can't read another site's HTML cross-origin — no data held, no secrets needed) and hands every other request straight to `env.ASSETS.fetch(request)`. `index.html`'s `fetchLinkPreviewInto()` calls that route from the trip's Links tab. This is the app's only server-side code — everything else in this doc that says "fully client-side" is still true of the app's actual data.
