...
Deployment is via Cloudflare Pages (`wrangler.toml`, `[assets] directory = "./"`). Pushing to `main` deploys to production at `or.rlapp.net`; other branches get Cloudflare's auto-generated preview URLs (`https://<branch>.<project>.pages.dev`). The app detects a `*.pages.dev` hostname at runtime and shows a visible red staging border so a preview is never mistaken for production.
...
The one exception is `functions/api/link-preview.js`, a small stateless Cloudflare Pages Function (auto-served alongside the static assets — no config beyond the file's existence). It does a single server-side `fetch` of a pasted link and returns its Open Graph title/description/image, purely because the browser itself can't read another site's HTML cross-origin; it holds no data and needs no secrets. `index.html`'s `fetchLinkPreviewInto()` calls it from the trip's Links tab. This is the app's only server-side code — everything else in this doc that says "fully client-side" is still true of the app's actual data.
